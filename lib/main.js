	...
	module.exports.setupWebSocket = function(server, backendApis, main, namespace){
		...
		nsp.on('connection', function (socket) {
			return new (function(socket){
				...
				// ユーザーに関する情報
				_this.bifloraUserInfo = {};
				while(1){
					_this.bifloraUserInfo.connectionId = +new Date();
					if( userList[_this.bifloraUserInfo.connectionId] ){
						continue;
					}
					userList[_this.bifloraUserInfo.connectionId] = _this.bifloraUserInfo;
					break;
				}
				console.log(_this.bifloraUserInfo);
				...
				_this.socket.on('disconnect', function () {
					...
					console.log('-- Leaved user:', _this.bifloraUserInfo);
					userList[_this.bifloraUserInfo.connectionId] = undefined;
					delete(userList[_this.bifloraUserInfo.connectionId]);
					console.log('-- userList:', userList);
					console.log('');
					try {
						if( backendApis['disconnect'] ){
							backendApis['disconnect']( _this.bifloraUserInfo, function(data){}, main, _this );
						}
					} catch (e) {
					}
				});
